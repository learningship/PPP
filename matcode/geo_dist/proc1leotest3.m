...
sat_slip = zeros(sat_N,L);
sat_slip1 = zeros(sat_N,L);
...
%           x(1:3)=rover_pos;
    ...
    dtrp = tropmodel(time, pos, azel, 0.7);
    vart = ERR_SAAS^2;
%     dion=0;
%     dtrp=0;
...
%      Xminus1(4) = X(i,4);
    ...
    Pminus1=Pplus1+blkdiag(1e-6*eye(3),1e-1,1e-2*eye(8));
    ...
    azel = satazel(pos, e1);
%    [dtrp,dtdx,vart] = trop_model_prec(time, pos,azel,Xminus1(5));
    dtrp = tropmodel(time, pos, azel, 0.7);
    vart = ERR_SAAS^2;
%      dion=0;
%      dtrp=0;
