...
% rover_pos = [-2176805.6923727235 4387453.6654526051 4071934.8767794245];
 ...
 rover_pos=[-2191672.7436-10  4417449.0579-10  4099773.9035-10 ];
...
L1=P1/(CLIGHT/FREQ1)+10;
L2=P2/(CLIGHT/FREQ2)+10;
...
Xplus1(1,1:4)=[-2173830  4381455  4093730 10];
...
%     Xplus1(1,1:4) = X(2,:);
    Xminus1=Xplus1(i-1,:);
%    Xplus1(i-1,5:12)=Lc(i-1)-Pc(i-1);
%      Xminus1(4) = X(i,4);
    Pminus1=Pplus1+blkdiag(1e-6*eye(3),1e-6,1e-3*eye(8));
    
        for s=1:8
    g0(s) = (L1(s,i-1)/FREQ1-L2(s,i-1)/FREQ2)*CLIGHT;
    g1(s) = (L1(s,i)/FREQ1-L2(s,i)/FREQ2)*CLIGHT;
    if(g0(s) ~=0 && abs(g0(s)-g1(s))>0.05)
        sat_slip(s,i)=1;
    else
        sat_slip(s,i)=0;
    end
    ...
    for s=1:8
    w0(s) = (L1(s,i-1)-L2(s,i-1))*CLIGHT/(FREQ1-FREQ2)-(FREQ1*P1(s,i-1)+FREQ2*P2(s,i-1))/(FREQ1+FREQ2);
    w1(s) = (L1(s,i)-L2(s,i))*CLIGHT/(FREQ1-FREQ2)-(FREQ1*P1(s,i)+FREQ2*P2(s,i))/(FREQ1+FREQ2);
    if(w0(s) ~=0 && abs(w0(s)-w1(s))>10)
        sat_slip1(s,i)=1;
    else
        sat_slip1(s,i)=0;
    end
