...
%ÂË²¨Ç°³õÖµ
Xplus1(1,1:4)=[-2173830  4381455  4093730 3];
Xplus1(1,5:11)=zeros(1,7);
 Xplus1(1,5:11)=(Lc(:,1)-Pc(:,1))';
Pplus1=30*eye(11);
...
%      Xminus1=Xplus1(i-1,:);
    Xminus1=Xplus1(i-1,:);
%      Xminus1(4) = X(i,4);
    Pminus1=Pplus1+blkdiag(1e-4*eye(3),10000,1e-1*eye(7));
    var1 = var;
    e2 = [];
    Ha = eye(7);
...
%     sat_clk = 0;
    ...
    v(j+2*(s-1)) = y - (r+cdtr-cdts+dtrp+bias);    
    var(j+2*(s-1)) = ((fact*0.003)^2+(fact*0.003/sin(azel(2)))^2)+vart+0.7*var1(2*s-1);
    end
    Ha = [Ha(1:2*s-1,:);zeros(1,7);Ha(2*s:end,:)];
    e2 = [e2;e1;e1];
    end
    R1 = diag(var);
    H1 = [e2,-ones(2*s,1),-Ha];
