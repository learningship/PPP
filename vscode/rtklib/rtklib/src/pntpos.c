                     ...
                     double *var)
{
    ...
    *var=0.0;
    ...
    if (opt->ionoopt==IONOOPT_IFLC) { /* dual-frequency */
		...
		else if (sys == SYS_LEO) {/*LEO*/
			gamma = SQR(FREQ1_LEO / FREQ2_LEO);
			return (P2-gamma*P1)/(1.0-gamma);
		}
    }
                   ...
                   double *resp, int *ns)
{
    ...
    for (i=*ns=0;i<n&&i<MAXOBS;i++) {
		...
		//else if (sys==SYS_LEO) {v[nv]-=x[8]; H[8+nv*NX]=1.0; mask[5]=1;}
        ...
        var[nv++]=varerr(opt,azel[1+i*2],sys)+vare[i]+vmeas+vion+0*vtrp;
...
{
	double x[NX] = { 0 }, dx[NX] = { 0 }, Q[NX*NX] = { 0 }, *v, *H, *var, sig, sta_pos[3] = { -2173820.0 , 4381460.0 , 4093740.0 };
	...
	if (norm(sol->rr, 3) <= 0)
	{
		for (i = 0; i < 3; i++) sol->rr[i] = sta_pos[i];
	}
    ...
    for (i=0;i<MAXITR;i++) {
        ...
        if (norm(dx,NX)<1E-4) {
            ...
            for (j=0;j<3;j++) sol->qr[j]=(float)Q[j+j*NX];/*Q[0] Q[9] Q[18] Q = H*H' (8*8)*/
