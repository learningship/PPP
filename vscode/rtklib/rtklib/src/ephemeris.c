                    ...
                    int ephopt, double *rs, double *dts, double *var, int *svh)
{
    ...
    for (i=0;i<n&&i<2*MAXOBS;i++) {
        ...
        if (dts[i*2]==0.0) {
            ...
            dts[1+i*2]=0.0;
            *var=SQR(STD_BRDCCLK)*0;//ÔÝÊ±Îª0
        }
    }
    FILE* fp1;
    if ((fp1 = fopen("sat_data.txt", "a")) == NULL)
    {
        printf("Wrong write.");
        exit(0);
    }
	...
	if (ii % 2 == 0) {
		for (i = 0; i < n&&i < 2 * MAXOBS; i++) {
			//if (obs[i].sat == 2 || obs[i].sat == 5 || obs[i].sat == 13 || obs[i].sat == 15 || obs[i].sat == 21 || obs[i].sat == 29 || obs[i].sat == 30) {
				fprintf(fp1, "%s sat%2d %13.3f %13.3f %13.3f %12.3f\n", time_str(time[i], 6),
					obs[i].sat, rs[i * 6], rs[1 + i * 6], rs[2 + i * 6], dts[i * 2] * 1E9);
			//}
