...
#define NUMSYS      8                   /* number of systems add LEO system*/
...
static const int navsys[]={             /* satellite systems add LEO*/
    SYS_GPS,SYS_GLO,SYS_GAL,SYS_QZS,SYS_SBS,SYS_CMP,SYS_IRN,SYS_LEO,0
};
static const char syscodes[]="GREJSCID"; /* satellite system codes add LEO(D)*/
                          ...
                          sigind_t *index, obsd_t *obs)
{
    ...
    if (ver>2.99) { /* ver.3 */
		if (!strncmp(buff, "D", 1)) 
		{
			sprintf(satid, "%.5s", buff);
			obs->sat = (uint16_t)satid2no(satid);
		}
		else 
		{
    ...
    switch (satsys(obs->sat,NULL)) {
		...
		case SYS_LEO: ind=index+7; break;//add leo obs
        default:      ind=index  ; break;
    }
	if (!strncmp(buff, "D", 1))
	{
		for (i = 0, j = ver <= 2.99 ? 0 : 5; i < ind->n; i++, j += 16) {
                      ...
                      char tobs[MAXOBSTYPE][4], sigind_t *ind)
{
    ...
    switch (sys) {
		...
		case SYS_LEO: optstr="-DL%2s=%lf"; break;
    }
                       ...
                       sta_t *sta)
{
	...
	if (nsys>=8) set_index(ver,SYS_LEO,opt,tobs[7],index+7);//add LEO index
...
static int readrnxclk(FILE *fp, const char *opt, int index, nav_t *nav)
{
    ...
    while (fgets(buff,sizeof(buff),fp)) {
		if (!strncmp(buff+3,"D",1)) {
			if (str2time(buff, 10, 26, &time)) {
				trace(2, "rinex clk invalid epoch: %34.34s\n", buff);
				continue;
			}
			strncpy(satid, buff + 3, 6);
			...
			for (i = 0, j = 42; i < 2; i++, j += 20) data[i] = str2num(buff, j, 19);
