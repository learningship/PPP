...
static char codepris[8][MAXFREQ][16]={  /* code priority for each freq-index */
   /*    0         1          2          3         4         5     */
    ...
    {"ABCX"    ,"ABCX"      ,""        ,""       ,""       ,""      ,""}, /* IRN */
	{"DCPYWMNSL","DPYWCMNDLSX","DIQX"     ,""       ,""       ,""      ,""}  /* LEO */
};
...
extern int satid2no(const char *id)
{
    ...
    switch (code) {
		...
		case 'D': sys= SYS_LEO;prn=(prn/100-1)*30+(prn-prn/100*100); break;
        default: return 0;
    }
    return satno(sys,prn);
}
...
/* LEO obs code to frequency -------------------------------------------------*/
static int code2freq_LEO(uint8_t code, double *freq)
{
	char *obs = code2obs(code);

	switch (obs[0]) {
	case '5': *freq = FREQ1_LEO; return 0; /* D1 */
	case '7': *freq = FREQ2_LEO; return 1; /* D2 */
	}
	return -1;
}
...
extern int code2idx(int sys, uint8_t code)
{
    ...
    switch (sys) {
		...
		case SYS_LEO: return code2freq_LEO(code,&freq);//add leo freq
    }
    return -1;
}
...
extern int getcodepri(int sys, uint8_t code, const char *opt)
{
    ...
    switch (sys) {
		...
		case SYS_LEO: i=7; optstr="-DL%2s"; break;
        default: return 0;
    }
    ...
    return (p=strchr(codepris[i][j],obs[1]))?16-(int)(p-codepris[i][j]):0;
}
...
extern void traceobs(int level, const obsd_t *obs, int n)
{
    ...
    for (i=0;i<n;i++) {
        ...
        //fprintf(fp, " (%2d) %s %-3s rcv%d %13.3f %13.3f %13.3f %13.3f %d %d %d %d %3.1f %3.1f\n",
        //    i + 1, str, id, obs[i].rcv, obs[i].L[0], obs[i].L[1], obs[i].P[0],
        //    obs[i].P[1], obs[i].LLI[0], obs[i].LLI[1], obs[i].code[0],
        //    obs[i].code[1], obs[i].SNR[0] * SNR_UNIT, obs[i].SNR[1] * SNR_UNIT);
    }
