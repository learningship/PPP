...
int staticcheckbrk(const char *format, ...)
{
	return checkbrk(format);
}
...
/* process positioning -------------------------------------------------------*/
void staticprocpos(FILE *fp, const prcopt_t *popt, const solopt_t *sopt,
	int mode)
{
	procpos(fp, popt, sopt, mode);
}
...
void staticfreeobsnav(obs_t *obs, nav_t *nav)
{
	freeobsnav(obs, nav);
}
...
void staticsetpcv(gtime_t time, prcopt_t *popt, nav_t *nav, const pcvs_t *pcvs,
	const pcvs_t *pcvr, const sta_t *sta)
{
	setpcv(time, popt, nav, pcvs, pcvr, sta);
}
...
void staticreadotl(prcopt_t *popt, const char *file, const sta_t *sta)
{
	readotl(popt, file, sta);
}
...
int staticouthead(const char *outfile, char **infile, int n,
	const prcopt_t *popt, const solopt_t *sopt)
{
	return outhead(outfile, infile, n,popt, sopt);
}
                   ...
                   const char *rov, const char *base)
{
    ...
    //strcpy((*fopt).satantp, infile[6]);
    //strcpy((*fopt).rcvantp, infile[6]);
