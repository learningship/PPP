...
static int code2sys(char code)
{
    ...
    if (code=='D') return SYS_LEO; /* SP3-d */
    return SYS_NONE;
}
/* read SP3 header -----------------------------------------------------------*/
                     ...
                     char *tsys, int index, int opt, nav_t *nav)
{
    ...
    while (fgets(buff,sizeof(buff),fp)) {
        ...
        for (i=pred_o=pred_c=v=0;i<n&&fgets(buff,sizeof(buff),fp);i++) {
			...
			if (sys == SYS_LEO) {
				//prn = (int)str2num(buff, 2, 4);
				prn = (int)(str2num(buff, 2, 2)-1) * 30 + (int)(str2num(buff, 4, 2));
				...
				for (j = 0; j < 4; j++) {
					...
					val = str2num(buff, 8 + j * 14, 14);
					std = str2num(buff, 64 + j * 3, j < 3 ? 2 : 3);
                    ...
                    double *rs, double *dts, double *var)
{
    ...
    /* relativistic effect correction 不考虑*/
    if (dtss[0]!=0.0) {
