...
#define THRES_REJECT 18.0            /* reject threshold of posfit-res (sigma) （原来为4.0）LEO需要修改*/
...
/*ΦM-W=(f1*Φ1-f2*Φ2)/(f1-f2)+c(N2-N1)/(f1-f2)-(f1*P1+f2*P2)/(f1+f2)
       =Φw*λw+Nw*λw-(f1*P1+f2*P2)/(f1+f2)*/
/*宽巷：n1=1,n2=-1,λw=c/(f1-f2)*/
static double mwmeas(const obsd_t *obs, const nav_t *nav)
{
    ...
    return (obs->L[0]-obs->L[1])*CLIGHT/(freq1-freq2)-
           (freq1*obs->P[0]+freq2*obs->P[1])/(freq1+freq2);//M-W组合(不包含模糊度）
}
